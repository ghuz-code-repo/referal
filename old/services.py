...
def _fetch_and_process_contacts_task(mysql_config, app_context):
    ...
    try:
        ...
        import traceback
        print(f"{task_name}: MySQL Error traceback: {traceback.format_exc()}")
    except Exception as e: 
        print(f"{task_name}: General Error during processing: {e}") 
        errors.append(f"General Error: {str(e)}")
        import traceback
        print(f"{task_name}: General Error traceback: {traceback.format_exc()}")
    finally:
        if connection:
            connection.close()
...
def _fetch_and_process_deals_task(mysql_config, app_context):
    """
    Fetches deals from MySQL and inserts them into the local MacroDeal table.
    Clears existing deals before importing new ones.
    """
